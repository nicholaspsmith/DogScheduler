...
function dosesInRange(start: string, end: string): Dose[] {
  const all: Dose[] = []
  for (let d = start; d <= end; d = addDays(d, 1)) all.push(...dosesForDay(SEED_MEDS, d))
  return all
}
...
describe('dose identity and shape', () => {
  it('IDs follow medId:YYYY-MM-DD:slot', () => {
    const ids = dosesForDay(SEED_MEDS, '2026-07-22').map((d) => d.id)
    expect(ids).toContain('prednisone:2026-07-22:am')
    expect(ids).toContain('clindamycin:2026-07-22:pm')
  })
  it('carries display fields', () => {
    const dose = dosesForDay(SEED_MEDS, '2026-07-21').find((d) => d.medId === 'prednisone')!
...
describe('pillInventories', () => {
  ...
  it('lists every dose id of the course, first to last', () => {
    const pred = pillInventories(SEED_MEDS).find((i) => i.medId === 'prednisone')!
...
describe('fluconazole (4 tablets/dose)', () => {
  it('yields exactly 21 AM doses, Jul 23 through Aug 12, nothing outside', () => {
    ...
    expect(byMed('fluconazole', dosesForDay(SEED_MEDS, '2026-07-22'))).toHaveLength(0)
    expect(byMed('fluconazole', dosesForDay(SEED_MEDS, '2026-08-13'))).toHaveLength(0)
  })
  it('doses on consecutive days with correct display fields', () => {
    const dose = byMed('fluconazole', dosesForDay(SEED_MEDS, '2026-07-30'))[0]
...
describe('heartworm monthly rule', () => {
  ...
  it('does not fire on Jul 14, 2026 (before rule start)', () => {
    expect(byMed('heartworm', dosesForDay(SEED_MEDS, '2026-07-14'))).toHaveLength(0)
  })
  it('continues indefinitely', () => {
    expect(byMed('heartworm', dosesForDay(SEED_MEDS, '2030-03-14'))).toHaveLength(1)
  })
})
...
describe('adequan monthly tail', () => {
  it('is day-of-month (11th), not every-28-days', () => {
    ...
    expect(byMed('adequan', dosesForDay(SEED_MEDS, '2026-09-08'))).toHaveLength(0)
  })
  it('weekly phase and monthly tail do not overlap in August', () => {
    // Monthly starts Sep 11; Aug 11 comes only from the weekly phase.
    expect(byMed('adequan', dosesForDay(SEED_MEDS, '2026-08-11'))).toHaveLength(1)
  })
})
